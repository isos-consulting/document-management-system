  ...
  <div data-role="page" data-theme="${Config.MOBILE_THEME}">
    ...
    <div data-role="content">
      <table style="width: 100%">
        <tr><td><b><u:message key="folder.uuid" module="${Translation.MODULE_MOBILE}"/>:</b></td><td>${fld.uuid}</td></tr>
        <tr><td><b><u:message key="folder.name" module="${Translation.MODULE_MOBILE}"/>:</b></td><td><u:getName path="${fld.path}"/></td></tr>
        <tr><td><b><u:message key="folder.parent" module="${Translation.MODULE_MOBILE}"/>:</b></td><td><u:getParent path="${fld.path}"/></td></tr>
        <tr><td><b><u:message key="folder.created" module="${Translation.MODULE_MOBILE}"/>:</b></td><td><u:formatDate calendar="${fld.created}"/> by ${fld.author}</td></tr>
        <c:choose>
          <c:when test="${fld.subscribed}"><c:set var="subscribed" value="Yes"/></c:when>
          <c:otherwise><c:set var="subscribed" value="No"/></c:otherwise>
        </c:choose>
        <tr>
          <td valign="top"><b><u:message key="folder.keywords" module="${Translation.MODULE_MOBILE}"/>:</b></td>
          <td>
            <c:forEach var="keyword" items="${fld.keywords}" varStatus="loop">
              ${keyword}<c:if test="${!loop.last}">,</c:if>
            </c:forEach>
          </td>
        </tr>
        <tr>
          <td valign="top"><b><u:message key="folder.categories" module="${Translation.MODULE_MOBILE}"/>:</b></td>
          <td>
        ...
        <c:if test="${not empty fld.subscriptors}">
          ...
          <tr>
            <td colspan="2">
              <div data-role="collapsible" data-collapsed="true" data-content-theme="c">
                <h3><u:message key="folder.subscribed.users" module="${Translation.MODULE_MOBILE}"/></h3>
        ...
        <c:if test="${not empty notes}">
          <tr>
            <td colspan="2">
              <div data-role="collapsible" data-collapsed="true" data-content-theme="c">
                <h3><u:message key="folder.notes" module="${Translation.MODULE_MOBILE}"/></h3>
                <ul data-role="listview">
